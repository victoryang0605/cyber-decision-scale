...
> 计费顺序：付费余额 → 免费额度；余额不足时自动回落到免费额度。

## 微信登录（用户注册体系）

配置后，用户须**微信授权**才能使用（自动注册：获取微信昵称/头像，手机号由用户手动绑定）。

**前置条件**：已认证的微信公众号（服务号/订阅号均可，网页授权需要已认证）。
1. 公众号后台 → 设置与开发 → 基本配置，获取 **AppID / AppSecret**（IP 白名单需填服务器出口 IP）
2. 公众号后台 → 设置与开发 → 公众号设置 → 功能设置 → **网页授权域名** 填你的站点域名
3. 配置环境变量：`WECHAT_APPID`、`WECHAT_APPSECRET`、`SESSION_SECRET`（强随机串）
4. 部署后用户需**在微信内打开**站点完成授权

**数据存储（CSV）**：登录用户的信息持久化到服务器 `data/` 目录：
- `users.csv`：openid、昵称、手机号、付费余额、已用/总免费次数、头像、时间
- `recharges.csv`：充值流水（人工确认后由管理员发放时写入）

**用户操作**：顶栏显示微信昵称/头像与「退出登录」；额度弹窗可绑定手机号、查看余额、**注销账号**（删除服务器上的用户与充值记录）。

> ⚠️ CSV 与微信登录依赖服务器可写文件系统 → 请以 **Zeabur（Express）部署为主站**。Cloudflare Pages（serverless，无文件系统）不含微信登录与 CSV，仅保留匿名额度逻辑。
